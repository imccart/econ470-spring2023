...
## Enrollment Data
Run the `R` code to organize the [Monthly Plan Enrollment Data](https://github.com/imccart/ma-class/blob/main/data-code/1_Plan_Data.R). Once you've created your final dataset (it's called *full_ma_data* in my code), answer the following:
...
5. Merge the contract service area data to the enrollment data, and restrict the data only to contracts that are approved in their respective counties. The `R` script to create the service area dataset is here: [Contract Service Area](https://github.com/imccart/ma-class/blob/main/data-code/3_Service_Areas.R). And you can follow the [_BuildFinalData.R](https://github.com/imccart/ma-class/blob/main/data-code/_BuildFinalData.R) script to see where/how I join the datasets. Limiting your dataset only to plans with non-missing enrollment data, provide a graph showing the average number of Medicare Advantage enrollees per county from 2008 to 2015. Be sure to format your graph in a meaningful way.
...
6. Merge the plan characteristics data to the dataset you created in Step 6 above. Note that you'll need to join the [Market Penetration Data](https://github.com/imccart/ma-class/blob/main/data-code/4_Penetration_Files.R) in order to get the information you need to merge the plan characteristics. This is because the plan characteristics data only have state name and county (not FIPS codes). The penetration files have both FIPS codes and state/county names, so that dataset serves as a good crosswalk file. Provide a graph showing the average premium over time. Don't forget about formatting!
